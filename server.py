...
async def store_in_kql(question: str, response: Dict, context: List[Dict], session_id: Optional[str] = None): 
...
async def get_from_kql_cache(question: str, session_id: Optional[str] = None) -> Optional[Dict]: 
...
async def get_latest_responses(session_id: Optional[str] = None) -> List[Dict]: 
...
async def add_smart_context_to_prompt(base_prompt: str, conversation_history: Optional[List[Dict]], question: str) -> str: 
...
SCHEMA_CACHE_DURATION = 3600  # Cache for 1 hour 
...
async def build_chatgpt_system_prompt(question: str, tables_info: List[Dict], conversation_history: Optional[List[Dict]] = None) -> str: 
...
async def cached_intelligent_analyze(question: str, session_id: Optional[str] = None) -> Dict[str, Any]: 
...
async def intelligent_analyze(question: str, session_id: Optional[str] = None) -> Dict[str, Any]: 
...
async def get_chat_messages( 
    ...
    try: 
        ...
        | take {limit * 2 if limit is not None else 20} 
    ...
    try: 
        ...
        for i, row in enumerate(raw_results): 
            ...
            try: 
                ...
                last_question = row["LastQuestion"] if "LastQuestion" in row else first_question 
